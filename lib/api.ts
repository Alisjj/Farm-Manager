...
export class ApiError extends Error {
  ...
  constructor(message: string, status: number) {
    ...
    this.name = 'ApiError';
    this.status = status;
    this.code = status === 403 ? 'FORBIDDEN' : status === 401 ? 'UNAUTHORIZED' : 'ERROR';
  }
...
export async function login(credentials: { username: string; password: string }) {
  ...
  const data = await handleResponse<{ token?: string; refreshToken?: string; user?: unknown }>(res);
  if (data?.token) {
...
export async function updateHouse(id: number | string, payload: HousePayload): Promise<House> {
...
export async function deleteHouse(id: number | string) {
  const res = await fetchWithAuth(`${BASE}/api/houses/${id}`, { method: 'DELETE' });
  return handleResponse(res);
}
...
export async function getDailyLogs(filters: Record<string, string> = {}): Promise<DailyLog[]> {
...
export async function createDailyLog(payload: DailyLogPayload): Promise<DailyLog> {
...
export async function updateDailyLog(id: number | string, payload: Partial<DailyLogPayload>): Promise<DailyLog> {
...
export async function deleteDailyLog(id: number | string) {
  const res = await fetchWithAuth(`${BASE}/api/daily-logs/${id}`, { method: 'DELETE' });
  return handleResponse(res);
}
...
export async function updateCustomer(id: number | string, payload: Partial<CustomerPayload>): Promise<Customer> {
...
export async function deleteCustomer(id: number | string) {
  const res = await fetchWithAuth(`${BASE}/api/customers/${id}`, { method: 'DELETE' });
  return handleResponse(res);
}
...
export async function getSales(filters: Record<string, string> = {}): Promise<Sale[]> {
  const params = new URLSearchParams(filters);
...
export async function updateSale(id: number | string, payload: Partial<SalePayload>): Promise<Sale> {
...
export async function deleteSale(id: number | string) {
  const res = await fetchWithAuth(`${BASE}/api/sales/${id}`, { method: 'DELETE' });
  return handleResponse(res);
}
...
export async function updateLaborer(id: number | string, payload: Partial<LaborerPayload>): Promise<Laborer> {
...
export async function deleteLaborer(id: number | string) {
  const res = await fetchWithAuth(`${BASE}/api/laborers/${id}`, { method: 'DELETE' });
  return handleResponse(res);
}
...
export async function getWorkAssignments(filters: Record<string, string> = {}): Promise<WorkAssignment[]> {
  const params = new URLSearchParams(filters);
...
export async function createWorkAssignment(payload: WorkAssignmentPayload): Promise<WorkAssignment> {
...
export async function updateWorkAssignment(id: number | string, payload: Partial<WorkAssignmentPayload>): Promise<WorkAssignment> {
...
export async function generatePayroll(monthYear: string): Promise<Payroll[]> {
  const res = await fetchWithAuth(`${BASE}/api/payroll/generate/${monthYear}`, { method: 'POST' });
  const data = await handleResponse<{ data?: Payroll[] }>(res);
  return data?.data || [];
}

export async function updatePayroll(id: number | string, payload: PayrollPayload): Promise<Payroll> {
...
export async function createFeedRecipe(payload: FeedRecipePayload): Promise<FeedRecipe> {
  const res = await fetchWithAuth(`${BASE}/api/feed/recipes`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
...
export async function updateFeedRecipe(id: number | string, payload: Partial<FeedRecipePayload>): Promise<FeedRecipe> {
...
export async function deleteFeedRecipe(id: number | string) {
  const res = await fetchWithAuth(`${BASE}/api/feed/recipes/${id}`, { method: 'DELETE' });
  return handleResponse(res);
}
...
export async function getFeedBatches(filters: Record<string, string> = {}): Promise<FeedBatch[]> {
  const params = new URLSearchParams(filters);
...
export async function createFeedBatch(payload: FeedBatchPayload): Promise<FeedBatch> {
...
export async function updateFeedBatch(id: number | string, payload: Partial<FeedBatchPayload>): Promise<FeedBatch> {
...
export async function deleteFeedBatch(id: number | string) {
  const res = await fetchWithAuth(`${BASE}/api/feed/batches/${id}`, { method: 'DELETE' });
  return handleResponse(res);
}

export async function getBatchIngredients(batchId: number | string): Promise<Ingredient[]> {
...
export async function addBatchIngredient(batchId: number | string, payload: Partial<Ingredient>): Promise<Ingredient> {
  const res = await fetchWithAuth(`${BASE}/api/feed/batches/${batchId}/ingredients`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
...
export async function calculateFeedBatchCost(payload: Partial<FeedBatchPayload>) {
...
export async function getFeedBatchUsageById(id: number | string): Promise<BatchUsageStats | null> {
...
export async function createCostEntry(payload: Partial<CostEntry>): Promise<CostEntry> {
...
export async function updateCostEntry(id: number, payload: Partial<CostEntry>): Promise<CostEntry> {
  const res = await fetchWithAuth(`${BASE}/api/cost-entries/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
...
export async function deleteCostEntry(id: number) {
  const res = await fetchWithAuth(`${BASE}/api/cost-entries/${id}`, { method: 'DELETE' });
  return handleResponse(res);
}
...
export async function getProductionReport(startDate: string, endDate: string): Promise<ProductionReportData> {
  const res = await fetchWithAuth(`${BASE}/api/reports/production?start=${startDate}&end=${endDate}`);
  const data = await handleResponse<{ data?: ProductionReportData }>(res);
  return data?.data || { start: startDate, end: endDate, days: 0, totalEggs: 0, avgPerDay: 0, logs: [] };
}

export async function getSalesReport(startDate: string, endDate: string): Promise<SalesReportData> {
  const res = await fetchWithAuth(`${BASE}/api/reports/sales?start=${startDate}&end=${endDate}`);
  const data = await handleResponse<{ data?: SalesReportData }>(res);
  return data?.data || { start: startDate, end: endDate, totalAmount: 0, totalEggs: 0, rows: [] };
}

export async function getFinancialReport(startDate: string, endDate: string): Promise<FinancialReportData> {
  const res = await fetchWithAuth(`${BASE}/api/reports/financial?start=${startDate}&end=${endDate}`);
  const data = await handleResponse<{ data?: FinancialReportData }>(res);
  return data?.data || { start: startDate, end: endDate, totalOperating: 0, totalSales: 0, ops: [], sales: [] };
}

export async function exportReport(type: 'production' | 'sales' | 'financial', format: 'csv' | 'pdf', startDate: string, endDate: string): Promise<Blob> {
  const res = await fetchWithAuth(`${BASE}/api/reports/export/${type}?format=${format}&start=${startDate}&end=${endDate}`);
  if (!res.ok) {
...
export async function getDashboardSummary(startDate: string, endDate: string): Promise<DashboardSummary> {
  ...
  const crackedEggs = production?.logs.reduce((sum, l) => sum + (l.crackedEggs || 0), 0) || 0;
  ...
  const paidCount = sales?.rows.filter(r => r.paymentStatus === 'paid').length || 0;
  const pendingCount = sales?.rows.filter(r => r.paymentStatus === 'pending').length || 0;
  const pendingPayments = sales?.rows.filter(r => r.paymentStatus === 'pending').reduce((sum, r) => sum + r.totalAmount, 0) || 0;

  const totalOperating = financial?.totalOperating || 0;
  const profitMargin = totalRevenue > 0 ? ((totalRevenue - totalOperating) / totalRevenue) * 100 : 0;
