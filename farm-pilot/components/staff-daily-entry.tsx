...
import { getHouses, createDailyLog, getFeedBatches } from '@/lib/api';
...
import { Calendar, Egg, Package, AlertTriangle, Clock, CheckCircle } from 'lucide-react';
...
export function StaffDailyEntry() {
    ...
    getHouses()
      .then((data: unknown[]) => {
        ...
        const normalized: House[] = data.map((h) => {
          ...
          return {
            ...
            status: (obj.status as 'active' | 'maintenance' | 'inactive') || 'active',
          } as House;
        });
      ...
      .catch(() => {
        ...
        if (savedHouses) {
          ...
          if (!mounted) return;
          setHouses(allHouses.filter((house: House) => house.status === 'active'));
        }
      })
  ...
  const feedTypes = ['Layer Mash', 'Grower Feed', 'Starter Feed', 'Finisher Feed'];
  ...
  const handleSubmitFeedLog = async () => {
    ...
    try {
      ...
      setFeedData({ amount: '', feedBatchId: '', type: '', notes: '' });
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-balance">Daily Farm Logging</h2>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Choose which house you&#39;re logging data for</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Record today&#39;s egg collection by grade</CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Record feed given to the selected house</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setFeedData((prev) => ({ ...prev, feedBatchId: value }))
                    }
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={feedData.type}
                    onValueChange={(value) => setFeedData((prev) => ({ ...prev, type: value }))}
                  >
                    <SelectTrigger>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Report any issues, concerns, or notable observations</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Textarea
