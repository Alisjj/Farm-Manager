...
const reportService = {
  ...
  getSalesReport: async (start, end) => {
    ...
    const totalDozens = rows.reduce(
      (s, r) =>
        s +
        (Number(r.gradeAQty || 0) +
          Number(r.gradeBQty || 0) +
          Number(r.gradeCQty || 0)),
      0
    );
    return { start, end, totalAmount, totalDozens, rows };
  },
  ...
  exportReportCsv: async (type, start, end) => {
    ...
    if (type === "production") {
      ...
      data = r.rows.map((s) => ({
        ...
        gradeAQty: s.gradeAQty,
        gradeBQty: s.gradeBQty,
        gradeCQty: s.gradeCQty,
      }));
