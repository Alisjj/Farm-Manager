...
export function ReportsSection() {
      ...
      if (sales && sales.rows.length > 0) {
        try {
          ...
          allSales.forEach(sale => {
            if (sale.customerId) {
              const existing = customerSalesMap.get(sale.customerId) || { orders: 0, revenue: 0 }
          ...
          setTopCustomers(customerSummaries.sort((a, b) => b.revenue - a.revenue).slice(0, 5))
        } catch {
          setTopCustomers([])
        }
      }
  ...
  // Handle export
  const handleExport = async (type: 'production' | 'sales' | 'financial', format: 'csv' | 'pdf') => {
    setExporting(true)
    const { startDate, endDate } = getDateRange()
  ...
  const crackedEggs = productionData?.logs.reduce((sum, l) => sum + (l.crackedEggs || 0), 0) || 0
  const crackedPercent = totalEggs > 0 ? Math.round((crackedEggs / totalEggs) * 100 * 10) / 10 : 0
  ...
  const paidTransactions = salesData?.rows.filter(r => r.paymentStatus === 'paid').length || 0
  const pendingTransactions = salesData?.rows.filter(r => r.paymentStatus === 'pending').length || 0
    ...
    <div className="flex gap-2">
      {canExport && (
        <Button 
          ...
          onClick={() => handleExport(reportType as 'production' | 'sales' | 'financial', exportFormat as 'csv' | 'pdf')}
          disabled={exporting}
        >
      ...
      <Tabs defaultValue="overview" className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">{Math.round(avgDaily)} eggs/day average</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">{totalDozens} dozens sold</p>
              </CardContent>
            </Card>
        ...
        <TabsContent value="production" className="space-y-6">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">Cracked Rate</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{crackedPercent}%</div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {productionData.logs.slice(0, 10).map((log, idx) => (
                      <TableRow key={idx}>
                        <TableCell className="font-medium">{new Date(log.logDate).toLocaleDateString()}</TableCell>
                        <TableCell>{(log.eggsCollected || 0).toLocaleString()}</TableCell>
        ...
        <TabsContent value="sales" className="space-y-6">
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{totalDozens.toLocaleString()}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">₦{Math.round(avgPricePerDozen).toLocaleString()}</div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {paidTransactions + pendingTransactions > 0 
                    ? Math.round((paidTransactions / (paidTransactions + pendingTransactions)) * 100)
                    : 0}%
                </div>
              </CardContent>
            </Card>
        ...
        <TabsContent value="financial" className="space-y-6">
          <div className="grid gap-4 md:grid-cols-3">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-chart-5">₦{totalRevenue.toLocaleString()}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-destructive">₦{totalOperatingCosts.toLocaleString()}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className={`text-2xl font-bold ${netProfit >= 0 ? 'text-chart-5' : 'text-destructive'}`}>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex justify-between items-center p-3 border rounded-lg">
                  ...
                  <span className="text-lg font-bold text-chart-5">₦{totalRevenue.toLocaleString()}</span>
                </div>

                <div className="flex justify-between items-center p-3 border rounded-lg">
                  <span className="font-medium">Total Operating Costs</span>
                  <span className="text-lg font-bold text-destructive">₦{totalOperatingCosts.toLocaleString()}</span>
                </div>
                ...
                <div className="flex justify-between items-center p-3 border rounded-lg">
                  ...
                  <Badge variant={profitMargin > 15 ? "default" : profitMargin > 0 ? "secondary" : "destructive"}>
                    {profitMargin}%
                  </Badge>
                </div>
              </div>
