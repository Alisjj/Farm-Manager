...
export function FeedManagement() {
  ...
  const loadFeedData = async () => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to load feed data';
      setError(message);
      setBatches([]);
      setBatchUsageStats([]);
    } finally {
  ...
  const addIngredient = () => {
    setNewBatch({
      ...
      ingredients: [
        ...
        { ingredientName: '', quantityKg: 0, totalCost: 0, supplier: '' } as IngredientInput,
      ],
    });
  };
  ...
  const calculateCost = async () => {
    try {
      ...
      const ingredientsWithCostPerKg: Ingredient[] = validIngredients.map((ing) => ({
        ...ing,
  ...
  const handleCreateBatch = async () => {
    try {
      ...
      setNewBatch({
        ...
        batchDate: new Date().toISOString().split('T')[0],
        bagSizeKg: 50,
        ingredients: [{ ingredientName: '', quantityKg: 0, totalCost: 0, supplier: '' } as IngredientInput],
      });
  ...
  const confirmDeleteBatch = async () => {
    ...
    try {
      ...
      await loadFeedData();
      showNotification('success', 'Batch Deleted', `Feed batch "${deleteConfirm.batchName}" has been deleted.`);
